<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.jianshenfangfengmian" label="健身房封面" prop="jianshenfangfengmian">
          <file-upload
          tip="点击上传健身房封面"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.jianshenfangfengmian?ruleForm.jianshenfangfengmian:''"
          @change="jianshenfangfengmianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.jianshenfangfengmian" label="健身房封面" prop="jianshenfangfengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jianshenfangfengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	if(this.ruleForm.jianshenfangfengmian!=null) {
		this.ruleForm.jianshenfangfengmian = this.ruleForm.jianshenfangfengmian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jianshenfang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jianshenfangCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    jianshenfangfengmianUploadChange(fileUrls) {
	this.ruleForm.jianshenfangfengmian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
