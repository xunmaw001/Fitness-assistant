import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import discussjianshenfang from '@/views/modules/discussjianshenfang/list'
    import jianshendaka from '@/views/modules/jianshendaka/list'
    import news from '@/views/modules/news/list'
    import xiangmumingcheng from '@/views/modules/xiangmumingcheng/list'
    import jianshenfang from '@/views/modules/jianshenfang/list'
    import discussjianshenshipin from '@/views/modules/discussjianshenshipin/list'
    import jianshenxiangmu from '@/views/modules/jianshenxiangmu/list'
    import yuyuexinxi from '@/views/modules/yuyuexinxi/list'
    import storeup from '@/views/modules/storeup/list'
    import forum from '@/views/modules/forum/list'
    import discussjianshenxiangmu from '@/views/modules/discussjianshenxiangmu/list'
    import jianshenshipin from '@/views/modules/jianshenshipin/list'
    import yonghu from '@/views/modules/yonghu/list'
    import richangreliang from '@/views/modules/richangreliang/list'
    import config from '@/views/modules/config/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/discussjianshenfang',
        name: '健身房评论',
        component: discussjianshenfang
      }
      ,{
	path: '/jianshendaka',
        name: '健身打卡',
        component: jianshendaka
      }
      ,{
	path: '/news',
        name: '系统公告',
        component: news
      }
      ,{
	path: '/xiangmumingcheng',
        name: '项目名称',
        component: xiangmumingcheng
      }
      ,{
	path: '/jianshenfang',
        name: '健身房',
        component: jianshenfang
      }
      ,{
	path: '/discussjianshenshipin',
        name: '健身视频评论',
        component: discussjianshenshipin
      }
      ,{
	path: '/jianshenxiangmu',
        name: '健身项目',
        component: jianshenxiangmu
      }
      ,{
	path: '/yuyuexinxi',
        name: '预约信息',
        component: yuyuexinxi
      }
      ,{
	path: '/storeup',
        name: '我的收藏管理',
        component: storeup
      }
      ,{
	path: '/forum',
        name: '健身论坛',
        component: forum
      }
      ,{
	path: '/discussjianshenxiangmu',
        name: '健身项目评论',
        component: discussjianshenxiangmu
      }
      ,{
	path: '/jianshenshipin',
        name: '健身视频',
        component: jianshenshipin
      }
      ,{
	path: '/yonghu',
        name: '用户',
        component: yonghu
      }
      ,{
	path: '/richangreliang',
        name: '日常热量',
        component: richangreliang
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
